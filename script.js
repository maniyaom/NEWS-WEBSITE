...
async function fetchNews(category,requiredArticles=5,json=[]) {
    ...
    // else if(query != null){
    //     result = await fetch(url + API_KEY + `&country=in&language=en&q=`+query);
    ...
    if(response_code == 429){
        console.log("failure !! and API_KEY_TRACKER = ",api_key_tracker);
...
async function loadMoreNews(category,requiredArticles=5,json=[]) {
    ...
    // else if(query != null)
    //     result = await fetch(url + API_KEY + `&country=in&language=en&q=`+query+`&page=`+nextPageId);
    else
